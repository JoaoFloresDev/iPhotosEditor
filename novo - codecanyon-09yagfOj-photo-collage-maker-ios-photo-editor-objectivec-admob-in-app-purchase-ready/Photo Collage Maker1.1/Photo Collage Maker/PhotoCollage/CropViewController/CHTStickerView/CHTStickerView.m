...
@property (nonatomic,retain) UITextView *dtextview;
...
@property (nonatomic,strong)UITapGestureRecognizer *textGesture;
...
- (UITapGestureRecognizer *)textGesture {
    if (!_textGesture) {
        _textGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleTextGesture:)];
        _textGesture.delegate = self;
    }
    return _textGesture;
}
...
- (UIImageView *)textImageview {
    if (!_textImageview) {
        _textImageview = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, defaultInset * 2, defaultInset * 2)];
        _textImageview.contentMode = UIViewContentModeScaleAspectFit;
        _textImageview.backgroundColor = [UIColor clearColor];
        _textImageview.userInteractionEnabled = YES;
        [_textImageview addGestureRecognizer:self.textGesture];
    }
...
-(void)setEnableText:(BOOL)enableText
{
    _enableText=enableText;
    if (self.showEditingHandlers) {
        [self _setEnableText:enableText];
    }
...
- (void)_setEnableText:(BOOL)enableText {
    self.textImageview.hidden = !enableText;
    self.textImageview.userInteractionEnabled = enableText;
}
...
- (id)initWithContentView:(UIView *)contentView {
  ...
  if (self = [super initWithFrame:frame]) {
    ...
    [self addSubview:self.flipImageView];
      [self setPosition:CHTStickerViewPositionBottomRight forHandler:CHTStickerViewHandlerText];
...
-(void)handleTextGesture:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(stickerViewDidText:)]) {
        [self.delegate stickerViewDidText:self];
    }
...
- (void)setImage:(UIImage *)image forHandler:(CHTStickerViewHandler)handler {
  switch (handler) {
    ...
    case CHTStickerViewHandlerFlip:
      ...
      break;
    case CHTStickerViewHandlerText:
...
- (void)setPosition:(CHTStickerViewPosition)position forHandler:(CHTStickerViewHandler)handler {
  ...
  switch (handler) {
    ...
    case CHTStickerViewHandlerFlip:
      ...
      break;
          case CHTStickerViewHandlerText:
          handlerView=self.textImageview;
  }
...
- (void)setHandlerSize:(NSInteger)size {
  ...
  [self setPosition:self.flipImageView.tag forHandler:CHTStickerViewHandlerFlip];
    self.textImageview.frame=handlerFrame;
    [self setPosition:self.textImageview.tag forHandler:CHTStickerViewHandlerText];
