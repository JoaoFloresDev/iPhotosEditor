...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!self.canAddPhoto
        || cell.isSelected) {
        NSLog(@"You can't select more images");
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:APPLICATION_NAME message:@"You can't Select More Images , If you can choose other image then please delete some image and choose other !!" delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
        [alert show];
        return NO;
    }
    if ([cell isKindOfClass:[YMSPhotoCell class]]) {
...
- (IBAction)finishPickingPhotos:(id)sender
{
    if (_selectIndex == 0) {
        if (_selectedPhotos.count == 1){
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:APPLICATION_NAME message:@"Please Select at least 2 Images for create collage picture" delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
            [alert show];
        }
        else{
            PhotoMainViewController *photo = [[PhotoMainViewController alloc]initWithNibName:@"PhotoMainViewController" bundle:nil];
            photo.MainimagesArray = self.selectedPhotos;
            photo.modalPresentationStyle = UIModalPresentationFullScreen;
            [self presentViewController:photo animated:YES completion:nil];
        }
    }
    else{
        if (_selectedPhotos.count == _numberOfPhotoToSelect) {
...
//            }
            ...
            tree.mainArray = self.selectedPhotos;
            tree.modalPresentationStyle = UIModalPresentationFullScreen;
            [self presentViewController:tree animated:YES completion:nil];
        }
        else{
            UIAlertView *Alert = [[UIAlertView alloc]initWithTitle:APPLICATION_NAME message:[NSString stringWithFormat:@"Please Select %lu Images",(unsigned long)_numberOfPhotoToSelect] delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
            [Alert show];
        }
    }
}
