...
class HKARSCNView: ARSCNView {
    ...
    func addPhotoRing_Box(vector3: SCNVector3, left: CGFloat, L: Int) {
        ...
        for index in 0..<L {
            ...
            if rescoucceConfiguration.box_Random {
                ...
                var i = Int(0)
                if index != 0 {
                    i = Int(index % rescouceManager.boxImages.count)
                }
    ...
    func addPhotoRing_Text(text: String, color: String) {
        ...
        // Iterar sobre cada caractere da string diretamente
        for t in text {
            index += 1
            let str = String(t)
            let textGeometry = SCNText(string: str, extrusionDepth: 0.1)
            textGeometry.font = UIFont.systemFont(ofSize: 0.4)
            textGeometry.firstMaterial?.diffuse.wrapT = .repeat
            let textNode = SCNNode(geometry: textGeometry)
            textNode.position = SCNVector3Make(0, 0, -ringRadius)
            textGeometry.firstMaterial?.diffuse.contents = UIImage(named: color)
            ...
            // Ajustar o ângulo baseado no comprimento do caractere
            if str.count == 3 {
